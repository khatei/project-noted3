...
static std::map<std::string,std::string> bufmap;


static int count = 0;

void takeinputtodisk(char* fname,char* buf)
{
...
void ImGui::ShowDemoWindow(bool* p_open)
{
 ...
 //code when New button is selected 
    if (ImGui::Button("New")) {
        add += 1;
        ...
        bufmap.insert(std::pair<std::string,std::string>(word_note,""));
    
    }


 // code when an item on the list is selected

        if (ImGui::ListBoxHeader("List new"))
        {
            for (auto& x : bufmap)
            {
                x.second.resize(3010);
                std::cout<<"now"<< std::endl;
                for (auto itr = bufmap.begin(); itr != bufmap.end(); itr++)
                {
                    std::cout << itr->first << "'\t" << itr->second;
                }

                ImGui::Selectable(x.first.c_str(), true);
                {
                    if (!ImGui::Begin(x.first.c_str(), &show_new_window))
                    {
                        ImGui::End();
                        return;
                    }
              ...
              //ImGui::InputTextMultiline(x.first.c_str(), buf, size_t(buf), ImVec2(-2.0f, ImGui::GetTextLineHeight() * 4));

                   // ImGui::InputTextMultiline(x.first.c_str(), const_cast<char *>(x.second), IM_ARRAYSIZE(const_cast<char *>(x.second)), ImVec2(-2.0f, ImGui::GetTextLineHeight() * 4));

                    //ImGui::InputTextMultiline(x.first.c_str(), x.second, IM_ARRAYSIZE(x.second), ImVec2(-2.0f, ImGui::GetTextLineHeight() * 4));
 ImGui::InputTextMultiline(x.first.c_str(),const_cast<char *>(x.second.c_str()),size_t(const_cast<char *>(x.second.c_str())), ImVec2(-2.0f, ImGui::GetTextLineHeight() * 4));

                   //char* output = const_cast<char *>(x.second.c_str());
                   //char* output = const_cast<char *>(x.second);
                    if (ImGui::Button("Save")) {
                        takeinputtodisk(const_cast<char *>(x.first.c_str()), const_cast<char *>(x.second.c_str()));
                    }
